...
const MenuTabPane: React.FC<MenuTabProp> = ({ firstStartIndex, firstEndIndex, secondStartIndex, secondEndIndex }) => {
	return (
		<div className='row'>
			<div className='col-md-6 menu-book-column'>
				{productList.slice(firstStartIndex, firstEndIndex).map((item) => (
					<div className='menu-list' key={item.id}>
						<div className='item'>
							<div className='img'>
								<Link href={`/menu/${item.slug}`} className='image-popup-vertical-fit' title='Mozzarella Dippers'>
				...
				{secondStartIndex !== undefined && secondEndIndex !== undefined
					...
					: productList.slice(secondEndIndex).map((item) => (
							<div className='menu-list' key={item.id}>
								<div className='item'>
									<div className='img'>
										<Link href={`/menu/${item.slug}`} className='image-popup-vertical-fit'>
											<img src={item.imgSrc} alt={item.name} />{' '}
										</Link>
									</div>
									<div className='item-content'>
